...
import { listCollections } from "../acc/sheets.client.js";
...
export function registerAccSheetsCollectionsGet(server: McpServer) {
  server.registerTool(
    "acc_sheets_collections_get",
    {
      title: "ACC Sheets - Collections",
      description:
        "Lista colecciones de Sheets para un proyecto.",
    ...
    async (args: InputArgs) => {
      ...
      if (page.schemaWarning) {
        warnings.push({
          ...
          source: "acc_sheets_collections_get"
        });
      }
      ...
      const payload = finalizePayload(
        ...
        buildMcpResponse({
          ...
          meta: {
            tool: "acc_sheets_collections_get",
            generatedAt: new Date().toISOString(),
            source: "construction/sheets/v1/projects/{projectId}/collections",
            projectResolution: {
