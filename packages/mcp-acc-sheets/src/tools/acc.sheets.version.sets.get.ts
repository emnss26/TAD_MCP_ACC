...
import { listVersionSets } from "../acc/sheets.client.js";
...
export function registerAccSheetsVersionSetsGet(server: McpServer) {
  server.registerTool(
    "acc_sheets_version_sets_get",
    {
      title: "ACC Sheets - Version Sets",
      description:
        "Lista version sets de un proyecto en Construction Sheets v1.",
    ...
    async (args: InputArgs) => {
      ...
      if (page.schemaWarning) {
        warnings.push({
          ...
          source: "acc_sheets_version_sets_get"
        });
      }
      ...
      const payload = finalizePayload(
        ...
        buildMcpResponse({
          ...
          meta: {
            tool: "acc_sheets_version_sets_get",
            generatedAt: new Date().toISOString(),
            source: "construction/sheets/v1/projects/{projectId}/version-sets",
            projectResolution: {
